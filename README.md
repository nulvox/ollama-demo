...
**Supports:** Bazzite Linux and Ubuntu/Debian, with NVIDIA and AMD GPU acceleration.
...
- Set up GPU acceleration (NVIDIA or AMD, if available)
...
| **NVIDIA Container Toolkit** | Lets containers use your NVIDIA GPU (skipped on AMD/CPU) |
...
Run `nvidia-smi` (NVIDIA) or `rocm-smi` (AMD) to check your VRAM, then pick accordingly:
...
| `.env` | Generated by `setup.sh` — sets the compose profile (`cpu`, `gpu`, or `amd`) |
